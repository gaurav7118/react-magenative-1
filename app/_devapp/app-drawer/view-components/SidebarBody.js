...
class SidebarBody extends Component {
    render() {
        ...
        const {hideIcon,hideArrowIcon,hideIntemBorder}=this.props.values;
        return (
            // bodyItems.map((item) => {
            ...
            <div className="dev-components dev-component-drawer user-account-options p-relative">
                <ul id="drawerItemContain2" className="drawer-options-contain">
                    {bodyItems.map((item,index) => (
                        <SidebarBodyElement name={item} key={index} hideIcon={hideIcon} hideArrowIcon={hideArrowIcon} hideIntemBorder={hideIntemBorder}/>
                    ))}
                </ul>
            </div>
        );
